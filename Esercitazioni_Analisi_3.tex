...
\input{../../../Libri/template_eserciziario.tex}

\usepackage{geometry}
\geometry{
	nomarginpar, % Toglie doppi margini
	margin=1.5in, % Imposta i margini a 1 inch
}
...
\pagestyle{empty} % Imposta lo stile senza niente finché non iniziano i contenuti
\vspace*{\fill}
%Teo Bucci \\
\begin{center}
	...
	\vspace*{1cm}
	{\large Di Norman Potrich e Marco Ugo Claudio Boella}\\
	\vspace*{0.1cm}
	{\large per il corso di Ingegneria Matematica\footnote{Questo testo non si intende come sostitutivo delle esercitazioni, sicuramente presenta degli errori, che potete gentilmente segnalare all'autore scrivendo a teobucci8@gmail.com. I contenuti sono di proprietà intellettuale degli esercitatori, mentre la trascrizione è di proprietà dell'autore.}}\\
	\vspace*{0.4cm}
	{\large Trascritte da Teo Bucci}\\
	\vspace*{0.1cm}
	{\large Aggiornato al \today}\\
	\vspace*{1cm}
...
quindi
\begin{align*}
( f*f)( x) & =\int _{\mathbb{R}} f( x-y) f( x) dx=\int ^{x}_{0} e^{-2( x-y)} e^{-2y} dy\\
...
\int _{\mathbb{R}}| f( x)| ^{2} dx & =\int ^{1}_{-1}\left( 1-x^{2}\right)^{2} dx=2\int ^{1}_{0}\left( 1-x^{2}\right)^{2} dx\\
 & =2\int ^{1}_{0}\left( 1-2x+x^{4}\right) dx=2\left[ x-\frac{2x^{3}}{3} +\frac{x^{5}}{5}\right]^{1}_{0} =\frac{16}{15}\\
 & \\
...
quindi la sua antitrasformata è
\begin{equation*}
g( t) =\frac{1}{2}(\sin( t) -t\cos( t)) H( t)
\end{equation*}
Funzione $K( s)$.

Notiamo che
\begin{equation*}
K( s) =\frac{1}{s}\mathcal{L}\{H( t)\sin( 2t)\} =\frac{1}{2}\mathcal{L}\left\{\int ^{t}_{0} H( \tau )\sin( 2\tau ) d\tau \right\} =\mathcal{L}\left\{\frac{1}{4}( 1-\cos( 2t)) H( t)\right\}
\end{equation*}
quindi la sua antitrasformata è
\begin{equation*}
