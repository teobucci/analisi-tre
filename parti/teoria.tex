...
\begin{equation*}
\begin{aligned}
v = (1, 0) = 1 & \ \ \Rightarrow \ \ \partial_{x}[u + iv] = u_{x} + iv_{x} = f'(z)\\
v = (0, 1) = i & \ \ \Rightarrow \ \ \partial_{y}[u + iv] = u_{y} + iv_{y} = if'(z)
\end{aligned}
\end{equation*}
...
\end{cases} \Rightarrow \ \ \partial_{yy} u + \partial_{xx} u = \Delta u = 0
\end{equation*}
Analogamente per $v$
\begin{equation*}
\Delta v = 0
...
allora
\begin{equation*}
e^{z} = w\ \ \Rightarrow \ \ e^{z + 2\pi i\cdot k} =| w| e^{i\arg w} \ \ \Rightarrow \ \ \log e^{z + 2\pi i\cdot k} = \log| w| e^{i\arg w}
\end{equation*}
...
\begin{equation*}
f\in H(A) \cap C^{0}(\overline{A}) \ \ \Rightarrow \ \ \int_{\gamma} f(z) dz = 0, \ \ \gamma = \partial A
\end{equation*}
...
\begin{itemize}
\item $(\Rightarrow)$ consideriamo $\gamma $ chiusa
\begin{equation*}
\int_{\gamma} f(z) dz = \int_{\gamma} F'(z) dz = \int^{b}_{a} F'(\gamma (t)) \gamma'(t) dt = F(\gamma (b)) - F(\gamma (a)) = 0
\end{equation*}
\item $(\Leftarrow)$ Consideriamo una certa $F$ definita da
\begin{equation*}
...
\Rightarrow \ \ A = \frac{1}{2\pi i}\int_{\gamma_{1}} f(w)\sum\limits^{\infty}_{n = 0}\frac{(z - z_{0})^{n}}{(w - z_{0})^{n + 1}} dw = \sum\limits^{\infty}_{n = 0} a_{n}(z - z_{0})^{n}
\end{gather*}
...
\begin{enumerate}
\item $\lim_{z\rightarrow z_{0}} f(z) = \infty \Leftrightarrow z_{0}$ è un polo.
\item $\lim_{z\rightarrow z_{0}} f(z)$ esiste finito $\Leftrightarrow z_{0}$ è una singolarità eliminabile.
\item $\lim_{z\rightarrow z_{0}} f(z)$ non esiste $\Leftrightarrow z_{0}$ è una singolarità essenziale.
\end{enumerate}
\end{thm}
\begin{thm}
...
\begin{equation*}
a_{n} = \frac{1}{2\pi i}\int_{\gamma}\frac{f(w)}{(w - z_{0})^{n + 1}} dw\ \ \Rightarrow \ \ \mathrm{Res}(f, z_{0}) = a_{- 1} = \frac{1}{2\pi i}\int_{\gamma} f(w) dw
\end{equation*}
...
\begin{equation*}
\boxed{\int_{\gamma} f(z) dz = 2\pi i\cdot \sum\limits^{n}_{j = 1}\mathrm{Res}(f, z_{j})}
\end{equation*}
\end{thm}
\begin{defn}
...
\begin{equation*}
\mathrm{Res}(f, \infty) : = - a_{- 1} = \frac{1}{2\pi i}\int_{- \gamma} f(w) dw
\end{equation*}
\end{defn}
\begin{thm}
...
\item se è polo di ordine $n$ allora $\mathrm{Res}(f, z_{0}) = \frac{1}{(n - 1)!}\lim\limits_{z\rightarrow z_{0}}\frac{d^{n - 1}}{dz^{n - 1}}(z - z_0)^n f(z)$
\item se è polo di ordine $1$ allora $\mathrm{Res}\left(\frac{f}{g}, z_{0}\right) = \lim\limits_{z\rightarrow z_{0}}\frac{f(z)}{g'(z)}$
...
\item se è zero di ordine $1$ allora $\mathrm{Res}(f, \infty) = - \lim\limits_{z\rightarrow \infty} zf(z)$
\item se è zero di ordine $\geq 2$ allora $\mathrm{Res}(f, \infty) = 0$
\end{itemize}
...
\end{rem}
\begin{rem}
Se $n = \mathrm{dim}(V) < \infty $, $\{e_{1}, \dotsc, e_{n}\}$, $(e_{j}, e_{k}) = \delta_{jk}$, allora $\forall v\in V\Rightarrow v = \sum^{n}_{k = 1} \lambda_{k} e_{k}$.
\end{rem}
\begin{rem}
Dire
\begin{equation*}
...
\begin{equation*}
\lambda \subset l^{2} \ \ \Rightarrow \ \ \sum\limits^{\infty}_{n = 1}| \lambda_{n}|^{2} < + \infty \ \ \Rightarrow \ \ \mu_{k} = \sum\limits^{k}_{n = 1}| \lambda_{n}|^{2} \ \ \text{è di Cauchy}
\end{equation*}
...
\Rightarrow \ \ x_{k} = \sum^{k}_{n = 1} \lambda_{n} e_{n} \ \ \text{è di Cauchy} \ \ \Rightarrow \ \ \text{converge per completezza.}
\end{gather*}
...
\begin{equation*}
\boxed{f\in L^{1} \ \ \Leftrightarrow \ \ \int_{A}| f(x)| dx < + \infty}
\end{equation*}
\end{defn}
...
\begin{align*}
\Rightarrow & | g(x)| \leq \Vert g \Vert_{L^{\infty}} \ \ \text{q.o. in} \ x\\
\Rightarrow & \int | f(x) g(x)| dx\leq \Vert g \Vert_{L^{\infty}}\int | f(x)| dx = \Vert g \Vert_{L^{\infty}} \Vert f \Vert_{L^{1}}
\end{align*}
...
\begin{equation}
\int | f(x) g(x)| dx\leq \frac{\lambda^{p - 1}}{p}\underbrace{\int | f(x)|^{p} dx}_{ \Vert f \Vert^{p}_{L^{p}}} + \frac{1}{\lambda q}\underbrace{\int | g(x)|^{q} dx}_{ \Vert g \Vert^{q}_{L^{q}}} \ \ \Rightarrow \ \ fg\in L^{1} \tag{*}
\end{equation}
...
\Rightarrow \ \ \Vert f + g \Vert_{L^{p}} \leq \Vert f \Vert_{L^{p}} + \Vert g \Vert_{L^{p}}
\end{gather*}
\end{proof}
\begin{thm}
...
\begin{equation*}
\int_{A} u(f_{n} - f) dx\rightarrow 0\ \ \ \ \Leftrightarrow \ \ \ \ \int_{A} uf_{n} dx\rightarrow \int_{A} ufdx
\end{equation*}quindi qualsiasi funzione $L^{1}_{\mathrm{loc}}$ è una distribuzione su quell'insieme.
\end{itemize}
\end{proof}

Si noti che lo spazio $L^{1}_{\mathrm{loc}}$ è molto più grande di $L^{1}$, ad esempio banalmente qualunque polinomio non identicamente nullo non è integrabile su tutto $\RR$, ma è integrabile su qualunque compatto.
...
Cosa vuol dire avere una successione di distribuzioni $\{\Lambda_{n}\}$ che converge a $\Lambda $ \underline{nel senso delle distribuzioni}? Come prima, se $\forall \varphi \in \Dc(A) \Rightarrow (\Lambda_{n}, \varphi)\rightarrow (\Lambda, \varphi)$. La definizione è simmetrica rispetto all'altra.
\end{defn}
...
NB. Quando diciamo che a $u\in L^{1}_{\mathrm{loc}}(A)$ corrisponde una distribuzione, diciame che esiste una $\Lambda_{u}$ distribuzione definita come $(\Lambda_{u}, \varphi) = \int_{A} u\varphi $, ma talvolta scriveremo direttamente $u$ con abuso di notazione.
\begin{defn}
...
\item continuo:
\begin{equation*}
\varphi_{k}\rightarrow \varphi \ \ \text{in} \ \Dc(A) \ \ \Rightarrow \ \ (\Lambda', \varphi_{k})\rightarrow (\Lambda', \varphi)
\end{equation*}
...
\begin{equation*}
(u, \varphi) = \int_{\RR} u\varphi dx = \int_{\RR} dx = + \infty \ \ \Rightarrow \ \ \text{non è temperata}
\end{equation*}
\begin{defn}
...
\begin{equation*}
g_{n}\rightarrow g\ \text{q.o.}, \ \exists h\in L^{1}, \ | g_{n}| \leq h\ \forall n\ \ \Rightarrow \ \ \lim\limits_{n\rightarrow + \infty}\int g_{n} = \int \lim\limits_{n\rightarrow + \infty} g_{n}
\end{equation*}
...
\begin{equation*}
\left| e^{i\lambda} - 1\right| \leq | \lambda | \ \ \lambda \in \RR \ \ \ \ \ \ \ \ \lambda = - xh\ \ \Rightarrow \ \ \text{l'integrale converge per conv. dom.}
\end{equation*}
\end{proof}
\begin{thm}
...
\Vert u \Vert^{2}_{L^{2}} & = \frac{1}{2\pi}\int \overline{u(x)}\int e^{ixz}\hat{u}(z) dzdx & \\
 ...
 & = \frac{1}{2\pi} \Vert \hat{u} \Vert^{2}_{L^{2}} \ \ \Rightarrow \ \ \Vert \hat{u} \Vert_{L^{2}} = \sqrt{2\pi} \Vert u \Vert_{L^{2}} &
\end{aligned}
\end{equation*}
...
allora
\begin{equation*}
\Fc\left\{\frac{1}{2\pi}\right\} = \delta_{0}(\xi) \ \ \Rightarrow \ \ \boxed{\Fc\{1\} = 2\pi \delta_{0}(\xi)}
\end{equation*}
...
\end{equation*}
Questo integrale ha senso purché $e^{- \lambda x} u(x) \in L^{1}(\RR)$, cioè $\mathrm{Re}(s) > \lambda (u)$, che è quindi il dominio della trasformata, mentre Fourier aveva senso sempre.
\end{defn}
\textbf{Proprietà}
\begin{itemize}
...
\end{equation*}
\item $\boxed{\lim\limits_{\mathrm{Re}(s)\rightarrow + \infty}\Lc(u(x), s) = 0}$

\begin{proof}
\begin{equation*}
\lim\limits_{\mathrm{Re}(s)\rightarrow + \infty}\Lc(u(x), s) = \lim\limits_{\mathrm{Re}(s)\rightarrow + \infty}\int^{+ \infty}_{0} e^{- s_{R} x} e^{- is_{I} x} u(x) dx
\end{equation*}

possiamo portare il limite dentro per convergenza dominata, purché $\mathrm{Re}(s) > \lambda (u)$ possiamo trovare una funzione che la domini
\begin{equation*}
\lim\limits_{\mathrm{Re}(s)\rightarrow + \infty}\int^{+ \infty}_{0} e^{- s_{R} x} e^{- is_{I} x} u(x) dx = \int^{+ \infty}_{0}\lim\limits_{\mathrm{Re}(s)\rightarrow + \infty} e^{- s_{R} x} e^{- is_{I} x} u(x) dx = 0
\end{equation*}
\end{proof}
...
\end{equation*}
\end{proof}
\item $\boxed{\Lc\left(e^{s_{0} x} u(x), s\right) = \Lc(u(x), s - s_{0})}$ $\boxed{\mathrm{Re}(s - s_{0}) > \lambda (u)}$
...
\begin{equation*}
\begin{aligned}
\Lc(u(cx), s) & = \int^{+ \infty}_{0} e^{- sx} u(cx) dx & x'= cx\ \ \Rightarrow \ \ dx'= cdx\\
...
\begin{equation*}
\boxed{\frac{d^{n}}{ds^{n}}\Lc(u(x), s) = (- 1)^{n} \Lc\left(x^{n} u(x), s\right)} \ \ \ \ \forall s\ \text{tale che} \ \mathrm{Re}(s) > \lambda (u)
\end{equation*}
\end{thm}
\begin{proof}
\begin{align*}
...
Inoltre
\begin{equation*}
\lim_{\mathrm{Re}(s)\rightarrow + \infty} s\Lc(u(x), s) = u(0)
\end{equation*}
Se $v\in L^{1}$, allora $\lambda (u) \leq 0$ e
\begin{equation*}
\lim\limits_{s\rightarrow 0, \ \mathrm{Re}(s) > 0} s\Lc(u(x), s) = \lim_{x\rightarrow + \infty} u(x)
\end{equation*}
\end{thm}
...
\item $\exists \lambda_{1}, \lambda_{2} \ \Rightarrow \ e^{- \lambda_{1} x} f(x) \in L^{1}(\RR), \ e^{- \lambda_{2} x} g(x) \in L^{1}(\RR)$
\end{itemize}
\begin{thm}
...
e^{- \lambda x}(f*g)(x) & = e^{- \lambda x}\int^{x}_{0} f(x - t) g(t) dt\\
 ...
 & = \left(e^{- \lambda x} f(x)\right) *\left(e^{- \lambda x} g(x)\right) \ \ \ \ \lambda \geq \max(\lambda (f), \lambda (g)) \ \Rightarrow \ \in L^{1}
\end{aligned}
\end{equation*}
...
\end{aligned}\\
\\
\Rightarrow \ \ \Lc(y(t), s) = \frac{\Lc(f(t), s) + a_{2}(sy_{0} + y_{1}) + a_{1} y_{0}}{a_{2} s^{2} + a_{1} s + a_{0}}
\end{gather*}
